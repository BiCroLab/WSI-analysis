...
node_color = pd.qcut(morphology_smooth.ravel(), quantiles, labels=False)
print('Done!')
...
if os.path.exists(outfile_covd) and os.path.exists(outfile_graph2covd):
    ...
    features = np.loadtxt(filename, delimiter="\t", skiprows=True, usecols=(5,6,colindex)).reshape((A.shape[0],3)) #including X,Y
...
logvec = [linalg.logm(m).reshape((1,6*6))  for m in flat_covdata] #calculate the logm and vectorize
