...
n2i = {}
if os.path.exists(outfile_n2i):
    n2i = np.load(outfile_n2i)
else:
    for n in unique_nodes:
        subg = [ g for g in subgraphs if ((n in g) and (len(g) <= maxthreshold)) ] #find the subgraphs containing a given n
...
###################################################################################################
subgraph = [g for g in subgraphs if len(g) <= maxthreshold]
print('Generate the covariance descriptor')
features = np.hstack((pos2norm,morphology_smooth))            # this is rotational invariant

outfile_covd_smallg = os.path.join(dirname, basename)+'.covd_smallg.npy'
outfile_graph2covd_smallg = os.path.join(dirname, basename)+'.graph2covd_smallg.npy'
if os.path.exists(outfile_covd_smallg) and os.path.exists(outfile_graph2covd_smallg):
    print('... loading the descriptors ...')
    covdata_smallg = np.load(outfile_covd_smallg,allow_pickle=True)
    graph2covd_smallg = np.load(outfile_graph2covd_smallg,allow_pickle=True)
else:
    print('... creating the descriptors ...')
    covdata_smallg, graph2covd_smallg = covd_multifeature(features,G,subgraphs) # get list of cov matrices and a list of nodes per matrix
    np.save(outfile_covd_smallg,covdata_smallg)
    np.save(outfile_graph2covd_smallg,graph2covd_smallg)

print('There are '+str(len(covdata_smallg))+' covariance descriptors ')
...
outfile_logvec_smallg = os.path.join(dirname, basename)+'.logvec_smallg.npy'
if os.path.exists(outfile_logvec_smallg):
    print('...load the logvec dataset...')
    X = np.load(outfile_logvec_smallg,allow_pickle=True)
else:
    print('...create the logvec dataset...')
    logvec_smallg = [linalg.logm(m).reshape((1,covdata_smallg[0].shape[0]*covdata_smallg[0].shape[1]))  for m in covdata_smallg] #calculate the logm and vectorize
    X_smallg = np.vstack(logvec_smallg) #create the array of vectorized covd data
    np.save(outfile_logvec_smallg,X_smallg)
    
outfile_clusterable_embedding_smallg = os.path.join(dirname, basename)+'.clusterable_embedding_smallg.npy'
if os.path.exists(outfile_clusterable_embedding_smallg):
    print('...load the clusterable embedding...')
    clusterable_embedding_smallg = np.load(outfile_clusterable_embedding_smallg,allow_pickle=True)
else:
    ...
    clusterable_embedding_smallg = umap.UMAP(min_dist=0.0,n_components=3,random_state=42).fit_transform(X_smallg) # this is used to identify clusters
    np.save(outfile_clusterable_embedding_smallg,clusterable_embedding_smallg)
